...
If the upload is not allowed, the server MUST return an appropriate HTTP status code and a custom header `X-Upload-Error` with a human readable error message.

Example for not allowed upload:

```http
HTTP/1.1 413 Forbidden
X-Upload-Error: File too large. Max allowed size is 100MB
```
