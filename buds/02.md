...
Servers MAY reject an upload for any reason and should respond with the appropriate HTTP `4xx` status code and and a custom header `X-Upload-Error` with a human readable error message.

Server MAY require payment for file storage. In that case, the server MUST return a **402 Payment Required** status code and 'Www-Authenticate' header with the macaroon and the payment request. See [Payment required](#payment-required)
...
The `HEAD /upload` endpoint MUST use the `Content-Type` and `Content-Length` headers to get the MIME type and size of the data that will be uploaded, returning a HTTP status code and a custom header `X-Upload-Error` to indicate if the upload is allowed or not.

The objective of this endpoint is to allow the client to check if the upload is allowed before sending the blob to the server, checking the size, mime type ond payment requirements if needed.
...
If the upload is not allowed, the server MUST return an appropriate HTTP status code and a custom header `Blossom-Upload-Error` with a human readable error message.

Example for not allowed upload:

```http
HTTP/1.1 413 Forbidden
Blossom-Upload-Error: File too large. Max allowed size is 100MB
```

If payment is required, the server MUST return a **402 Payment Required** status code and 'Www-Authenticate' header with the macaroon and the payment request. See [Payment required](#payment-required)

## Payment required

Some servers may require payment for file storage. In that case, the server SHOULD return the response with a **402 Payment Required** status code and 'Www-Authenticate' with the macaroon and the payment request fields using [L402 protocol](https://l402.org/) standard.

Example header response from the server for a paid blob:
...
Clients SHOULD check the `Www-Authenticate` header and display the payment request to the user. When the payment is done, the client SHOULD send a new request with the macaroon and the payment preimage in the `Www-Authenticate` header.
...
The server `MUST` return check the macaroon and the payment preimage and return the blob if the payment is done using a [blob descriptor](#blob-descriptor).


Until the payment is done, the server SHOULD display a QR code, JSON file (or another usefull information) instead of the blob on the response body. When the payment is done, the server MUST swap this information for the real the uploaded file. If the payment expires, the server SHOULD delete the blob and return a 404 status code.