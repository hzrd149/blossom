...
Using the `Authorization` HTTP header, the authorization token SHOULD be encoded as unpadded Base64URL and use the `Nostr` authorization scheme. Servers and clients MAY also support reading other Base64 encodings for compatibility with legacy implementations.
